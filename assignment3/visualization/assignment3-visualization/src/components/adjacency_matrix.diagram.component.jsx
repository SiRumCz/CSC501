...
export class AdjacencyMatrix extends Component {
    ...
    render_adjacency_matrix = () => {
        ...
        miserables.links = miserables.links.map(link => {
            let newLink = { source: 0, target: 0, value: 0};
            for(let i = 0; i < miserables.nodes.length; i++) {

                if(link.source.id === miserables.nodes[i].id) {
                    newLink.source = miserables.nodes[i].index;
                }
                if(link.target.id === miserables.nodes[i].id) {
                    newLink.target = miserables.nodes[i].index;
                }
            }
